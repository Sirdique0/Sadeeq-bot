...
loginForm.addEventListener('submit',async e=>{e.preventDefault();const btn=loginForm.querySelector('button');btn.disabled=true;status('Signing in...');const email=$('email').value.trim();const password=$('password').value;const {data,error}=await client.auth.signInWithPassword({email,password});if(error||!data.session){status('Login failed. Check your email and password.','error');btn.disabled=false;return}window.location.href='./dashboard.html';});
signupForm.addEventListener('submit',async e=>{e.preventDefault();const email=$('signup-email').value.trim();const password=$('signup-password').value;const confirm=$('signup-confirm').value;if(password.length<8){status('Password must be at least 8 characters.','error');return}if(password!==confirm){status('Passwords do not match.','error');return}const btn=signupForm.querySelector('button');btn.disabled=true;status('Creating the initial owner account...');if(!(await ownerSignupAvailable())){status('Initial Owner Signup is already closed.','error');setSignupAvailability(false);btn.disabled=false;return}const {data,error}=await client.auth.signUp({email,password});if(error){status(error.message,'error');btn.disabled=false;return}if(data.session){status('Owner account created successfully.','ok');setTimeout(()=>{window.location.href='./dashboard.html'},500)}else{status('Account created. Confirm your email, then use Login.','ok');showLogin()}btn.disabled=false;});
